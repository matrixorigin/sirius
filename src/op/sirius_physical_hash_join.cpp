...
static cudf::filtered_join make_filtered_join(cudf::table_view const& right,
                                              rmm::cuda_stream_view stream)
{
#if CUDF_VERSION_MAJOR > 26 || (CUDF_VERSION_MAJOR == 26 && CUDF_VERSION_MINOR >= 10)
  return cudf::filtered_join(right, cudf::null_equality::UNEQUAL, stream);
#else
  return cudf::filtered_join(
    right, cudf::null_equality::UNEQUAL, cudf::set_as_build_table::RIGHT, stream);
#endif
}
                                                                  ...
                                                                  rmm::cuda_stream_view stream)
{
  ...
  if (_join_mode == HASH_JOIN_MODE::BUILD_PROBE) {
    ...
    if (join_type == duckdb::JoinType::INNER) {
      ...
      auto filtered_join_object = make_filtered_join(right_keys, stream);
      left_indices              = filtered_join_object.semi_join(left_keys, stream);
    } else if (join_type == duckdb::JoinType::RIGHT_SEMI) {
      auto filtered_join_object = make_filtered_join(left_keys, stream);
